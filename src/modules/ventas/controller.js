// src/modules/ventas-detallado/controller.js
...
const { getVentasDetHTML } = require('./service');
const { parseVentasDetallado } = require('./parser');
...
async function ventasDetJSON(req, res) {
  ...
  try {
    ...
    const html = await getVentasDetHTML(client, {
      regionId,
      ubicacionId,
...
async function ventasDetHTML(req, res) {
  const user = (req.body?.user || UPPER_USER || '').trim();
  const pass = (req.body?.pass || UPPER_PASS || '').trim();
...
module.exports = { ventasDetJSON, ventasDetHTML };
